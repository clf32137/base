...
compactly stored in the array a[1..n][1..m1+m2+1] as described in the comment for routine banmul, 
this routine constructs an LU decomposition of a rowwise permutation of A. The upper triangular matrix replaces a, 
while the lower triangular matrix is returned in al[1..n][1..m1]. 
indx[1..n] is an output vector which records the row permutation effected by the partial pivoting; 
...
**************************************************************/
{
	...
	*d = 1.0;
	...
	for (k = 1; k <= n; k++) //For each row.
	{
		...
		indx[k] = i;
		if (dum == 0.0) a[k][1] = TINY; //Matrix is algorithmically singular. But proceed anyway with TINY pivot (desirable in some applications).
		if (i != k) 
		{							  //Interchange rows.
			*d = -(*d);
