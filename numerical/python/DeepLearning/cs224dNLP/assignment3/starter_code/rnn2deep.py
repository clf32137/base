...
class RNN2:
    ...
    def costAndGrad(self,mbdata,test=False): 
        ...
        return \
            ...
            [
                self.dL,
                scale * (self.dW1 + self.rho*self.W1),
                scale * self.db1,
    ...
    def forwardProp(self, node, correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            node.hActs2 = relu(np.dot(self.W2, node.hActs1) + self.b2)#TODO: Investigate if we can do this just once. 
        else:
        ...
        node.probs = y_hat
        cost -= np.log(y_hat[node.label]) #Labels are the tagged sentiments.
        node.fprop = True
    ...
    def backProp(self, node, error = None):
        ...
        # dU
        self.dWs += np.outer(delta3, node.hActs2)

        delta2 = np.dot(self.Ws.T, delta3) * (node.hActs2 > 0)
        self.db2 += delta2
        self.dW2 += np.outer(delta2, node.hActs1)

        #Since this is the lowest layer, the errors from previous layers will be aggregated here.
    ...
    def check_grad(self, data, epsilon = 1e-7):
        ...
        for W,dW in zip(self.stack[1:],grad[1:]):
            ...
            for i in xrange(W.shape[0]):
                for j in xrange(W.shape[1]):
                    ...
                    err1 += err
                    count += 1
        if 0.00001 > err1/count:
            print "Grad Check Passed for dW: Sum of Error = %.9f" % (err1/count)
        else:
